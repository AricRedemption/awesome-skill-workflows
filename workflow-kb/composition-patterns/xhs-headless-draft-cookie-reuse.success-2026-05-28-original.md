...
9. Switch to `图文笔记` and verify the target title appears in the image-draft list.
10. Keep the same browser process alive.
11. Recheck `草稿箱 -> 图文笔记` in the same browser process before handoff or downstream confirmation.
12. For visible inspection, reopen the same protected profile with injected cookies and verify the same title under `草稿箱 -> 图文笔记`.
13. Record only sanitized proof fields: `headless=true`, `clicked_publish=false`, `draftbox_detected=true`, `image_draft_count`, `title_found_in_draftbox=true`, whether this was a same-browser live-session check, and whether same-profile visible-browser verification passed.
