...
export function Header({ currentPath }: { currentPath: string }) {
  return (
    <header className="sticky top-3 z-10 mx-auto mt-3 w-full max-w-[1240px] rounded-[14px] border border-[#d1bf94]/20 bg-[#10120f]/80 px-3.5 py-3 shadow-[0_22px_70px_rgba(0,0,0,0.34)] backdrop-blur-[18px] max-[1272px]:w-[calc(100%-32px)]">
      <div className="grid grid-cols-[auto_1fr_auto] items-center gap-6 max-[700px]:grid-cols-[1fr_auto]">
        <a className="flex items-center gap-2.5 font-bold text-[#f7f1df]" href="/" aria-label={`${brand.name} home`}>
          ...
          <span className="min-w-0">
            ...
            <span className="block text-[11px] font-semibold text-[#efe5c7]/48 max-[700px]:hidden">{brand.repositoryName}</span>
          </span>
        </a>
          ...
          {navItems.map((item) => (
            <a
              className={`transition hover:text-[#f7f1df] ${
                item.href === currentPath ? "text-[#f7f1df]" : ""
              }`}
              key={item.href}
              href={item.href}
      ...
      <nav className="mt-3 hidden gap-2 overflow-x-auto pb-1 text-[12px] text-[#efe5c7]/70 max-[980px]:flex" aria-label="Mobile navigation">
        {navItems.map((item) => (
          <a
            className={`shrink-0 rounded-full border px-3 py-2 transition ${
              item.href === currentPath
                ? "border-[#d6ad69]/28 bg-[#d6ad69]/10 text-[#f7f1df]"
                : "border-[#d1bf94]/14 bg-[#f4e6c8]/4 hover:border-[#d6ad69]/24 hover:text-[#f7f1df]"
            }`}
            key={item.href}
            href={item.href}
            rel={item.href.startsWith("http") ? "noreferrer" : undefined}
